...
def make_panel(data_dict, nrow, ncol, idx, obs_dic, verb_month, hemisph, proj):
    ...
    for n_o, obs in enumerate(obs_dic.keys()):
        obs_scat_p = ax.scatter(obs_dic[obs]['mean'], obs_dic[obs]['dec_slope'],
                                label='OBS: '+obs.split('-')[1], s=200, marker="*", zorder=2,
    ...
    for n, model in enumerate(sorted(data_dict.keys())):
        ...
        if 'CanESM' in model:
            mod_obs_p = ax.errorbar(data_dict[model]['mean'], data_dict[model]['dec_slope'],
                        ...
                        c=sty['color'], marker=sty['mark'], ms=7, zorder=3, label=model)
    ...
    if hemisph == 'NH':
        ...
        ax.set_xlim(-0.1, 1)
        ax.set_yticks(np.arange(-0.2, 0.06, 0.05))
        y_text = -0.18
        x_text = 0.5
...
def main(cfg):
    ...
    data_dict = {'NH': {'CMIP6': {}}, 'SH': {'CMIP6': {}}, 'MH': {'CMIP6': {}}}
    ...
    for hemisph in data_dict.keys():
        ...
        for project in data_dict[hemisph].keys():
            models = set(dtsts.get_info_list('dataset', project=project, variable_group='siconc_cmip_'+hemisph.lower()))
            for model in models:
                ens_fnames = dtsts.get_info_list('filename', dataset=model, project=project, variable_group='siconc_cmip_'+hemisph.lower())
        ...
        obses = dtsts.get_info_list('dataset', project='OBS', variable_group='siarea_obs_' + hemisph.lower())
        data_dict[hemisph]['OBS'] = {}
        for obs in obses:
            obs_fname = dtsts.get_info('filename', dataset=obs, project='OBS', variable_group='siarea_obs_' + hemisph.lower())
