...
#Convert values in K to C.
  ...
  if 'siconca' in nc.variables:
    sic = np.ma.filled(nc.variables["siconca"][:,:,:],-1.0e30)
  else:
    sic = np.ma.filled(nc.variables["siconc"][:,:,:],-1.0e30)
    nc.close()

  # read sftlf.nc
  nc = netCDF4.Dataset(sftlf_file, "r")
  sftof = 1-np.ma.filled(nc.variables["sftlf"][:,:],-1.0e30) #NPG - added '1-' to use lf.
  nc.close()
  ...
  if 'm' in options:
    ...
    if tas.shape[0]>obs_tas.shape[0]:
      obs_tas = np.concatenate((obs_tas,np.full((tas.shape[0]-obs_tas.shape[0],obs_tas.shape[1],obs_tas.shape[2]),fill_value=-1e30)))
    ...
    cvgmsk = np.ma.filled(obs_tas,-1.0e30)
    nc.close()
  ...
  # dates
  dates = (np.arange(tas.shape[0])+0.5)/12.0 + y0

  # force missing cells to be open water/land and scale if stored as percentage
  sic[sic<  0.0] = 0.0
  sic[sic>100.0] = 0.0
  if np.max(sic)>90.0: sic = 0.01*sic

  sftof[sftof<  0.0] = 0.0
  sftof[sftof>100.0] = 0.0
  if np.max(sftof)>90.0: sftof = 0.01*sftof
  ...
  if 'a' in options:
    ...
    # prepare missing
    for m in range(sic.shape[0]):
      tos[m,abs(tos[m,:,:])> 500.0] = np.nan

    # baseline
    mask = np.logical_and( dates > 1961, dates < 1991 )
    base = tas[mask,:,:]
    for m in range(12):
      norm = np.mean(base[m::12,:,:],axis=0)
