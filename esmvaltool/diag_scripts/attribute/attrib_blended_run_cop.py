...
def attrib_warming(beta,betaCI,dec_warming,ci90_dec_warming,ci90_beta_obs=0.):
  ...
  attrib_range=[attrib_warming-np.absolute(beta)*dec_warming*(((beta-betaCI[0])/beta)**2+(ci90_dec_warming/dec_warming)**2+(ci90_beta_obs/beta)**2)**0.5,attrib_warming+np.absolute(beta)*dec_warming*(((betaCI[1]-beta)/beta)**2+(ci90_dec_warming/dec_warming)**2+(ci90_beta_obs/beta)**2)**0.5]
  return (attrib_warming,np.sort(attrib_range))
...
#Reorder longitudes to match HadCRUT4.
  ...
  for m in range(12):
    norm = np.nanmean(had4_base[m::12,:,:],axis=0)
...
# Define colours
    colors=np.array([[0,0,0],[196,121,0],[178,178,178],[0,52,102],[0,79,0],[200,0,0],[0,200,0],[0,0,200],[112,160,205],])/256.
    ...
    years=list(np.arange(1850+av_yr/2,2020+av_yr/2,av_yr)) #Used for plotting.
    ...
    mean_dec_warming=np.zeros((nexp,nmodel))
    ci90_dec_warming=np.zeros((nexp,nmodel))
    anom=np.zeros((ldiag,anom_max))
    anom_mod=np.zeros((anom_max))
    anom_index=0
    ens_sizes=np.zeros((nexp,nmodel))
    ensobs_diag=[]
    ensobs_dec_warming=[]
    ci90_beta_obs2=np.zeros((2))
    ci90_beta_obs3=np.zeros((3))
    ...
    for mm, dataset in enumerate(grouped_input_data):
        ...
        for exp in grouped_model_input_data:
            ...
            if nens==1:
                ci90_dec_warming[experiment,mm]=ci90_dec_warming[np.nonzero(ci90_dec_warming*(ens_sizes**0.5))].mean() #use mean of CIs already calculated, corrected for ensemble size if ensemble size is 1.
            else:
                ci90_dec_warming[experiment,mm]=(np.std(exp_dec_warming,ddof=1)/((nens)**0.5))*t.ppf(0.95,nens-1)
    ...
    #Carry out multi-model analysis with each ensemble member of ensemble obs dataset if flag is set.
      ...
      beta2=np.zeros((2,enssize))
      beta3=np.zeros((3,enssize))
      neff=nmodel**2/np.sum(1./ens_sizes[0:3,:],axis=1) #Effective ensemble size when using multi-model mean.
      ...
      for ee in range(enssize):
        ...
        plt.subplot(121)
        plt.plot([ee+0.9,ee+0.9],np.transpose(att_out2['betaCI'][0,:]),color=colors[1,:],linewidth=0.5)
        plt.plot([ee+1.1,ee+1.1],np.transpose(att_out2['betaCI'][1,:]),color=colors[4,:],linewidth=0.5)
        ...
        plt.subplot(122)
        plt.plot([ee+0.8,ee+0.8],np.transpose(att_out3['betaCI'][2,:]),color=cols[2,:],linewidth=0.5)
        plt.plot([ee+1.0,ee+1.0],np.transpose(att_out3['betaCI'][1,:]),color=cols[1,:],linewidth=0.5)
        plt.plot([ee+1.2,ee+1.2],np.transpose(att_out3['betaCI'][0,:]),color=cols[0,:],linewidth=0.5)
      ...
      for experiment in range(2):
        ci90_beta_obs2[experiment]=np.std(beta2[experiment,:],ddof=1)*t.ppf(0.95,enssize-1)
      for experiment in range (3):
        ci90_beta_obs3[experiment]=np.std(beta3[experiment,:],ddof=1)*t.ppf(0.95,enssize-1)
...
# Replace beta confidence interval bounds with large positive and negative numbers if NaNs.
        ...
        att_out3[dataset]['betaCI'][np.isnan(att_out3[dataset]['betaCI'][:,0]),0]=-1000.
        att_out3[dataset]['betaCI'][np.isnan(att_out3[dataset]['betaCI'][:,1]),1]=1000.
        att_out[dataset]['betaCI'][np.isnan(att_out[dataset]['betaCI'][:,0]),0]=-1000.
        att_out[dataset]['betaCI'][np.isnan(att_out[dataset]['betaCI'][:,1]),1]=1000.
...
#2-way regression coefficients.
        plt.subplot(topleft)
        plt.plot([mm_attrib+0.9,mm_attrib+0.9],np.transpose(att_out[dataset]['betaCI'][0,:]),color=colors[1,:],linewidth=2,label=ant)
        plt.plot([mm_attrib+1.1,mm_attrib+1.1],np.transpose(att_out[dataset]['betaCI'][1,:]),color=colors[4,:],linewidth=2,label=nat)
...
#3-way regression coefficients.
        ...
        plt.plot([mm_attrib+0.8,mm_attrib+0.8],np.transpose(att_out3[dataset]['betaCI'][2,:]),color=cols[2,:],linewidth=2,label=label[2])
        plt.plot([mm_attrib+1.0,mm_attrib+1.0],np.transpose(att_out3[dataset]['betaCI'][1,:]),color=cols[1,:],linewidth=2,label=label[1])
        plt.plot([mm_attrib+1.2,mm_attrib+1.2],np.transpose(att_out3[dataset]['betaCI'][0,:]),color=cols[0,:],linewidth=2,label=label[0])
...
#Compute uncertainty in attributable warming based on spread in ratio of GSAT to GMST warming across models.
    if diag_name[0:4]=='hemi':
      nlat=2
      nper=ldiag//2
      mean_diag=np.mean(np.reshape(mean_diag,(nlat,nper,nexp,nmodel)),axis=0)
    mean_dec_warming_gmst=np.squeeze(np.mean(mean_diag[32:34,:,:],axis=0)-np.mean(mean_diag[0:10,:,:],axis=0)) #Assume 5-yr means, calculate 2010-2019-1850-1899 in GMST.
    multim_mean_dec_warming_gmst=np.mean(mean_dec_warming_gmst[:,model_indices],axis=1)
    ...
    multim_ci90_dec_warming=np.std(mean_dec_warming/mean_dec_warming_gmst,ddof=1,axis=1)*t.ppf(0.95,nmodel-1)*np.mean(mean_dec_warming_gmst,axis=1)
  ...
  #3-way regression coefficients.
      plt.subplot(topright)
      plt.plot([mm_attrib+0.8,mm_attrib+0.8],np.transpose(att_out3[dataset]['betaCI'][2,:]),color=cols[2,:],linewidth=2)
      plt.plot([mm_attrib+1.0,mm_attrib+1.0],np.transpose(att_out3[dataset]['betaCI'][1,:]),color=cols[1,:],linewidth=2)
      plt.plot([mm_attrib+1.2,mm_attrib+1.2],np.transpose(att_out3[dataset]['betaCI'][0,:]),color=cols[0,:],linewidth=2)
      plt.plot([mm_attrib+0.8],att_out3[dataset]['beta'][2],color=cols[2,:],marker='+')
      plt.plot([mm_attrib+1.0],att_out3[dataset]['beta'][1],color=cols[1,:],marker='+')
      plt.plot([mm_attrib+1.2],att_out3[dataset]['beta'][0],color=cols[0,:],marker='+')
