...
def plot_heatwave_length(obs_cb: Cube, ref_cb: Cube, clim_cb: Cube, 
    ...
    shape_ids_fr ={
        'N.Quebec': 'le nord du Québec',
        'S.Quebec': 'le sud du Québec',
        'E.Ontario': "l’est de l’Ontario",
        'Atl.Canada': 'le Canada Atl.',
        'Atl.Canada': 'le Canada Atl.',
        'Kivalliq': 'le Kivalliq',
        'Ft.Smith': 'le Ft.Smith',
        'Inuvik': 'le Inuvik',
        'Kitikmeot': 'le Kitikmeot',
        'Yukon': 'le Yukon', 
        'N.BC': 'le nord de la C.B.',
        'S.BC': 'le sud de la C.B.',
        'Alberta': "l'Alberta",
        'Manitoba': 'le Manitoba',
        'Sask.': 'le Saskatchewan'
    }
    ...
    ax.plot(np.arange(ref_cb.shape[0]), ref_cb.data, label='Heatwave threshold | Seuil de canicule'
                                                            , c='darkgrey', zorder=2)
    ax.plot(np.arange(clim_cb.shape[0]), clim_cb.data, label='Climatology | Climatologie (' + str(cfg['reference_period'][0]) + '-' + \
    ...
    ax.set_xlabel('Date (month/day) | (mois/jour)')
    ax.set_xlim(0, plot_end_idx-ext_start_idx)
    ax.text(0.26, 0.02,'Heatwave length: ' +str(hw_info['hw_len'])+' days ('+ \
                                    str(hw_start)[5:].replace('-', '/')+' - '+\
                                    str(hw_end)[5:].replace('-', '/')+')\n'+\
                                    f'Climatology exceeded by {t_exceed}$^o$C', 
                                    transform=ax.transAxes)
    ax.text(0.62, 0.02,'Durée de la canicule: ' +str(hw_info['hw_len'])+' jours ('+ \
                                    str(hw_start)[5:].replace('-', '/')+' - '+\
                                    str(hw_end)[5:].replace('-', '/')+')\n'+\
                                    f'Climatologie dépassée par {t_exceed}$^o$C', 
                                    transform=ax.transAxes)
    ...
    fig.suptitle(f'{dataset} temperature in {shape_id} in '+y_lbl[1:-1]+f'\nTempérature {dataset} dans '+shape_ids_fr[shape_id]+ ' en '+y_lbl[1:-1], fontsize='x-large')
    # fig.suptitle(f'{dataset} temperature in {shape_id} in '+y_lbl[1:-1], fontsize='x-large')
    fig.tight_layout()
    fig.savefig(os.path.join(cfg['plot_dir'], 
                            f'hw_{shape_id}_{dataset}_fr.'+cfg['output_file_type']))
...
def main(cfg):
    ...
    for dataset in datasets.keys():
        ...
        dataset_csv_w.writerow(['Region', 'start_day', 'end_day',
                                                'max_day', '3_day_max',
                                                'length', 'clim_exceedance', 'obs_value'])
